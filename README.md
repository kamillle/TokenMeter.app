...
「Status」タブはOpenAIとAnthropicの公式公開ステータスAPIを5分ごとに確認し、全体状態、UsageBarに関連するサービス、進行中のインシデントを表示します。障害中のサービスは関連サービスの絞り込みにかかわらず表示します。右上の更新ボタンで手動更新でき、取得に失敗したプロバイダーは直前の表示を保持します。各カードとインシデントから公式ページを開けます。
