...
ClaudeのアカウントIDは、Claude Codeがローカルの `~/.claude.json` に保存したメールアドレスを表示します（メールがなければアカウントUUID）。`CLAUDE_CONFIG_DIR` が指定されている場合は、そのディレクトリ内の `.claude.json` だけを参照します。30秒ごとの集計時に読み直し、項目がない場合や読み取りに失敗した場合は「未取得」と表示します。これはローカルに保存されたアカウント情報で、現在の認証状態を検証するものではありません。設定の変更や認証情報の複製は行いません。

Claudeの枠はPro/Maxの応答後に通知されます。API・プロキシ等の接続先では通知されない場合があります。通知がない場合は **未取得／通知待ち** になります。トークン・料金の集計は機能します。

## プロバイダー ステータス

タイトル右隣のアイコンボタンで、棒グラフの「TokenMeter」と波形の「Status」を切り替えます。選択中のアイコンは色と背景で示し、マウスを重ねると画面名が表示されます。Statusに障害情報がある場合はStatusアイコンの右上に赤い点が付きます（点滅テスト中も同様）。

「Status」タブは、起動時・5分ごと・タブへ切り替えたときにOpenAIとAnthropicの公式公開ステータスAPIを確認し、全体状態、TokenMeterに関連するサービス、進行中のインシデントを表示します。障害中のサービスは関連サービスの絞り込みにかかわらず表示します。右上の更新ボタンでも再取得でき、取得に失敗したプロバイダーは直前の表示を保持します。各カードとインシデントから公式ページを開けます。タブを閉じていても定期取得します。障害情報がある間はメニューバー表示の左側で赤い丸が点滅し、正常状態の取得後に消えます。取得失敗時は直前の障害表示も保持します。
...
赤い丸の点滅を試す場合は、常駐中のTokenMeterを終了してから `open TokenMeter.app --args --demo-issue` で起動します。60秒後に実際の障害状態に基づく表示へ戻ります。
