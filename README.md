...
2. 項目内のどこをクリックしても、利用枠・リセット時刻と、セッション別の入力／出力トークン数・参考料金が見られます。INPUT／OUTPUT／参考 USD の見出しをクリックすると、降順と昇順が切り替わります。並び替えを解除して更新日時順へ戻すには「クリア」ボタンを使います。詳細画面ではデータの有無にかかわらずCodex／Claudeを切り替えられます。
...
集計はアプリ内のSwift処理で30秒ごとに実行します。初回はログを読み、以後は追記分を読みます。キャッシュは `~/Library/Application Support/UsageBar/` に保存します。保存内容はセッション名・作業ディレクトリ・使用量等の集計メタデータと、単価確認の日時・結果です。会話本文や認証情報は保存せず、外部通信は公式料金表と利用枠の読み取りに限定します。セッションデータを外部に送信しません。
